...
//----- meta information retrieval -------------------------------------------------------------------------------------
    ...
    public boolean hasState(String state)
    {
        ...
        if ( mMetaInformation.get(state)!=null ){
            return true;
        }
        else {
            return false;
...
//----- value access ---------------------------------------------------------------------------------------------------
     ...
     *
     * @result The parameter value for the empty key. Or nil if there's no empty key.
     */
     ...
     * @result The parameter value string for the given key. Or nil if there's no empty key set.
     */
     ...
     *
     * @result The parameter object for the empty key. Or nil if there's no empty key.
     */
     ...
     *
     * @param key The key for the value to be retrieved.
     * @result The parameter object for the given subkey. Or nil if there's no empty key set.
     */
...
//----- typed access and type conversions ------------------------------------------------------------------------------
     ...
     *
     * @result The NSNumber or nil if not found or not convertible.
     */
     ...
     *
     * @result The NSNumber or the default if not found or not convertible.
     */
 ...
 *
 * @result The UIColor or nil if not found or not convertible.
 */
     ...
     *
     * @result The UIColor or the default if not found or not convertible.
     */
     ...
     *
     * @result The float or 0.0f if not found or not convertible.
     */
     ...
     *
     * @result The float or the default if not found or not convertible.
     */
     ...
     *
     * @result The CGPoint 0.0f/0.0f if not found or not convertible.
     */
    ...
    public void convertToBool(boolean removeNonConvertibles)
    {
        ...
        for (String key: mParameters.keySet() ) {
            ...
            if ( object instanceof String) {
                ...
                if ( ((String) object).equalsIgnoreCase("false")) {value=false; valid=true;}
                if ( ((String) object).equalsIgnoreCase("1")) {value=true; valid=true;}
                if ( ((String) object).equalsIgnoreCase("true")) {value=true; valid=true;}
                else {valid=false;}
                // replace (if the number is nil, it gets removed)
                if (valid) {
                    addObject(value,key);
